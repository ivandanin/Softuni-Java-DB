...
-- find all info about departmetns
...
SELECT CONCAT(`first_name`, '.', `last_name`, '@softuni.bg') 
 ...
 -- gind all different employee's salaries
...
-- fins all employees without manager
SELECT `first_name`, `last_name` FROM `employees`
WHERE `manager_id` IS NULL;

-- find all employees where salary >= 50k
SELECT `first_name`, `last_name`, `salary` FROM `employees`
WHERE `salary` > 50000
...
WHERE `job_title` IN ('Engineering' OR 'Tool Design' OR 'Marketing' OR 'Information Serivces');
