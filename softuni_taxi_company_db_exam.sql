...
-- regular clients
SELECT c.`full_name`, COUNT(c.`id`) AS `count_of_cars`,
...
-- full info of courses
SELECT a.`name`,
IF (HOUR(cr.`start`) BETWEEN 6 AND 20, 'Day', 'Night') AS `day_time`,
SUM(`bill`), cl.`full_name`, c.`make`, c.`model`, cat.`name` AS `category_name`
FROM `addresses` AS a
JOIN `courses` AS cr
...
-- find all courses by clients phone number
DELIMITER //
