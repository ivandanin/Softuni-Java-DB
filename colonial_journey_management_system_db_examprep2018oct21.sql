...
SELECT (CASE
    ...
    THEN CONCAT_WS('',YEAR(`birth_date`), DAY(`birth_date`), SUBSTRING(c.`ucn`, 1, 4))
    ELSE CONCAT_WS('', YEAR(`birth_date`), MONTH(`birth_date`), SUBSTRING(c.`ucn`, 7, 10))
    END) AS `card_number`,
    (CASE
...
SELECT ships.`name` AS `spaceship_name`, 
ports.`name` AS `spaceport_name`
FROM `spaceships` AS ships
JOIN `journeys` AS j
ON ships.`id` = j.`spaceship_id`
JOIN `spaceports` AS ports
ON ports.`id` = j.`destination_spaceport_id`
ORDER BY `light_speed_rate` DESC
LIMIT 1;
 
-- extract pilots younger than 30
...
ON p.`id` = s.`planet_id`
JOIN `journeys` AS j
ON j.`destination_spaceport_id` = s.`id`) AS pl
...
SELECT j.`id`, p.`name` AS `planet_name`, s.`name` AS `spaceport_name`,
